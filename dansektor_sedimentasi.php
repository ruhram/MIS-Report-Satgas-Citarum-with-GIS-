...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-mountain mr-2"></i> Sedimentasi Tanah Sungai Citarum</h3>
            <hr>
            <div id="mapid"></div>
            <h3><i class="fas fa-mountain mr-2"></i> Statistik Sedimentasi Tanah Sungai Citarum</h3>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM sedimentasi INNER JOIN wilayah_subsektor ON sedimentasi.id_subsektor = wilayah_subsektor.id_subsektor WHERE sedimentasi.nama_sungai = 'citarum' ORDER BY sedimentasi.id_subsektor ASC";
                ...
                echo "
                    ...
                    <td>$row[kecamatan]</td>
                    <td>$row[kelurahan]</td>
                    <td>$row[volume_sedimentasi] m<sup>3</sup></td>
                    </tr>";
                    $no++;
                }
            ?>
   ...
   crossorigin=""></script>
    ...
    <script>
        ...
        var label = [];
        <?php
            $query = "SELECT * FROM sedimentasi INNER JOIN wilayah_subsektor ON sedimentasi.id_subsektor = wilayah_subsektor.id_subsektor WHERE sedimentasi.nama_sungai = 'citarum'";
            $sql = mysqli_query($conn, $query);
        ...
        <?php
            ...
            while($rows = mysqli_fetch_array($sql)){
        ?>
            var volume =  <?php echo $rows['volume_sedimentasi'] ?>;
            volumes.push(volume);
        <?php } ?>
        ...
        //script untuk peta tematik    
            ...
            info.update = function (props) {
                this._div.innerHTML = '<h4>Volume Sedimentasi Sungai Citarum</h4>' +  (props ?
            ...
            <?php
                ...
                $sql = mysqli_query($conn, "SELECT * FROM sedimentasi WHERE id_subsektor = 602");
                $sedimentasi2 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM sedimentasi WHERE id_subsektor = 605 && nama_sungai = 'citarum'");
                $sedimentasi5 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM sedimentasi WHERE id_subsektor = 606");
                $sedimentasi6 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM sedimentasi WHERE id_subsektor = 609");
                $sedimentasi9 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM sedimentasi WHERE id_subsektor = 611");
                $sedimentasi11 = mysqli_fetch_array($sql);
            ?>
            var sedimentasi = {"Bojongsari":<?=$sedimentasi2['volume_sedimentasi']?>, "Bojongsoang":<?=$sedimentasi5['volume_sedimentasi']?>,"Wargamekar":<?=$sedimentasi6['volume_sedimentasi']?>,"Baleendah":<?=$sedimentasi9['volume_sedimentasi']?>,"Oxbow Cijagra":<?=$sedimentasi11['volume_sedimentasi']?>}
            // get color depending on population density value
            function getColor(d) {
                return  d > 80000  ? '#7D2715' :
                        d > 70000  ? '#FC4E2A' :
                        d > 50000   ? '#FD8D3C' :
                        d > 30000   ? '#FEB24C' :
                        d > 10000   ? '#FED976' :
            ...
            function style(feature) {
		        return {
                    ...
                    dashArray: '3',
                    fillOpacity: 0.7,
                    fillColor: getColor(sedimentasi[feature.properties.kelurahan])
                };
	        }
            ...
            legend.onAdd = function (map) {
                ...
                var div = L.DomUtil.create('div', 'info legend'),
                    grades = [0, 10000, 30000, 50000, 70000, 80000],
                    labels = [],
                    from, to;
