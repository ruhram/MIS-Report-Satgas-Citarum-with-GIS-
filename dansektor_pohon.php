...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-tree mr-2"></i> Pohon</h3>
            <hr>
            <div id="mapid"></div>
            <br>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM pohon ORDER BY id_pohon ASC";
    ...
    <script>
        ...
        //script untuk peta tematik    
            ...
            info.update = function (props) {
                this._div.innerHTML = '<h4>Jumlah Pohon</h4>' +  (props ?
	        ...
	        info.addTo(map);
            <?php
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 1");
                $pohon1 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 602");
                $pohon2 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 604");
                $pohon4 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 606");
                $pohon6 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 607");
                $pohon7 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 608");
                $pohon8 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT SUM(jumlah_pohon) AS jumlah FROM pohon WHERE id_subsektor = 611");
                $pohon11 = mysqli_fetch_array($sql);
            ?>
            var pohon = {"Tegalluar": <?= $pohon1['jumlah']?>, "Bojongsari":<?=$pohon2['jumlah']?>, "Citeureup":<?=$pohon4['jumlah']?>, "Wargamekar":<?=$pohon6['jumlah']?>,"Jelekong":<?=$pohon7['jumlah']?>,"Manggahang":<?=$pohon8['jumlah']?>,"Oxbow Cijagra":<?=$pohon11['jumlah']?>}
            ...
            function style(feature) {
		        return {
                    ...
                    dashArray: '3',
                    fillOpacity: 0.7,
                    fillColor: getColor(pohon[feature.properties.kelurahan])
                };
	        }
