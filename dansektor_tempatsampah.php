...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-trash mr-2"></i> Tempat Sampah</h3>
            <hr>
            <div id="mapid"></div>
            <br>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM tempatsampah ORDER BY id_tempatsampah ASC";
    ...
    <script>
        ...
        //script untuk peta tematik    
            ...
            info.update = function (props) {
                this._div.innerHTML = '<h4>Jumlah Tempat Sampah</h4>' +  (props ?
	        ...
	        info.addTo(map);
            <?php
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 1");
                $tempatsampah1 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 602");
                $tempatsampah2 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 603");
                $tempatsampah3 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 604");
                $tempatsampah4 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 605");
                $tempatsampah5 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 606");
                $tempatsampah6 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 607");
                $tempatsampah7 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 608");
                $tempatsampah8 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 609");
                $tempatsampah9 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 610");
                $tempatsampah10 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM tempatsampah WHERE id_subsektor = 611");
                $tempatsampah11 = mysqli_fetch_array($sql);
            ?>
            var Tps = {"Tegalluar": <?= $tempatsampah1['jumlah']?>, "Bojongsari":<?=$tempatsampah2['jumlah']?>, "Cikoneng":<?=$tempatsampah3['jumlah']?>, "Citeureup":<?=$tempatsampah4['jumlah']?>, "Bojongsoang":<?=$tempatsampah5['jumlah']?>,"Wargamekar":<?=$tempatsampah6['jumlah']?>,"Jelekong":<?=$tempatsampah7['jumlah']?>,"Manggahang":<?=$tempatsampah8['jumlah']?>,"Baleendah":<?=$tempatsampah9['jumlah']?>,"Cibisoro":<?=$tempatsampah10['jumlah']?>,"Oxbow Cijagra":<?=$tempatsampah11['jumlah']?>}
            ...
            function style(feature) {
		        return {
                    ...
                    dashArray: '3',
                    fillOpacity: 0.7,
                    fillColor: getColor(Tps[feature.properties.kelurahan])
                };
	        }
