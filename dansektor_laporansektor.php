...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-parachute-box mr-2"></i> Taman</h3>
            <hr>
            <div id="mapid"></div>
            <br>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM taman ORDER BY id_taman ASC";
    ...
    <script>
        ...
        //script untuk peta tematik    
            ...
            info.update = function (props) {
                this._div.innerHTML = '<h4>Jumlah Taman</h4>' +  (props ?
	        ...
	        info.addTo(map);
            <?php
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 1");
                $taman1 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 602");
                $taman2 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 604");
                $taman3 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 605");
                $taman4 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 606");
                $taman5 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 607");
                $taman6 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 609");
                $taman7 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM taman WHERE id_subsektor = 611");
                $taman8 = mysqli_fetch_array($sql);
            ?>
            var Taman = {"Tegalluar": <?= $taman1['jumlah']?>, "Bojongsari":<?=$taman2['jumlah']?>, "Citeureup":<?=$taman3['jumlah']?>, "Bojongsoang":<?=$taman4['jumlah']?>, "Wargamekar":<?=$taman5['jumlah']?>,"Jelekong":<?=$taman6['jumlah']?>,"Baleendah":<?=$taman7['jumlah']?>,"Oxbow Cijagra":<?=$taman8['jumlah']?>}
            ...
            function style(feature) {
		        return {
                    ...
                    dashArray: '3',
                    fillOpacity: 0.7,
                    fillColor: getColor(Taman[feature.properties.kelurahan])
                };
	        }
