...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-dumpster-fire mr-2"></i> Incinerator</h3>
            <hr>
            <div id="mapid"></div>
            <br>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM incinerator ORDER BY id_incinerator ASC";
    ...
    <script>
        ...
        //script untuk peta tematik    
            ...
            info.update = function (props) {
                this._div.innerHTML = '<h4>Jumlah Incinerator</h4>' +  (props ?
	        ...
	        info.addTo(map);
            <?php
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 1");
                $tungku1 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 602");
                $tungku2 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 603");
                $tungku3 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 604");
                $tungku4 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 605");
                $tungku5 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 606");
                $tungku6 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 607");
                $tungku7 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 608");
                $tungku8 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 609");
                $tungku9 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 610");
                $tungku10 = mysqli_fetch_array($sql);
                $sql = mysqli_query($conn, "SELECT * FROM incinerator WHERE id_subsektor = 611");
                $tungku11 = mysqli_fetch_array($sql);
            ?>
            var Tungku = {"Tegalluar": <?= $tungku1['jumlah']?>, "Bojongsari":<?=$tungku2['jumlah']?>, "Cikoneng":<?=$tungku3['jumlah']?>, "Citeureup":<?=$tungku4['jumlah']?>, "Bojongsoang":<?=$tungku5['jumlah']?>,"Wargamekar":<?=$tungku6['jumlah']?>,"Jelekong":<?=$tungku7['jumlah']?>,"Manggahang":<?=$tungku8['jumlah']?>,"Baleendah":<?=$tungku9['jumlah']?>,"Cibisoro":<?=$tungku10['jumlah']?>,"Oxbow Cijagra":<?=$tungku11['jumlah']?>}
            ...
            function style(feature) {
		        return {
                    ...
                    dashArray: '3',
                    fillOpacity: 0.7,
                    fillColor: getColor(Tungku[feature.properties.kelurahan])
                };
	        }
